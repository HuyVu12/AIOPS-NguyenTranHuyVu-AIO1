...
# ── Load data ────────────────────────────────────────────────────────────────
alert_sample = pd.read_json(
    'dataset/alerts_sample.jsonl', lines=True
...
G = nx.DiGraph()
for svc in services:
    G.add_node(svc['name'], criticality=svc['criticality'])
for store in stores:
    G.add_node(store['name'], criticality=store['criticality'])
for e in edges:
...
summary = correlate(alert_sample, G, gap_sec=30, max_hop=2)
