...
#       container_name. Services in this lab have container_name set, so
#       scaling to N>1 is illustrative. Real production use would target
#       a service without a fixed container_name or use Swarm/K8s.
#
# Exit codes:
...
# Resolve the docker-compose.yml relative to this script's location
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *) echo "[scale_replicas] Unknown argument: $1"; exit 1 ;;
  esac
done
...
echo "[scale_replicas] Scale command issued for $SERVICE → $REPLICAS replicas. SUCCESS."
exit 0
